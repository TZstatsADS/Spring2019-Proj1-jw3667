...
# change text to cleaned_hm !?!?!?
# remove (months_3 = "3m",) in line 71, due to less data
hm_data <- hm_data %>%
...
```{r bag of words, warning=FALSE, message=FALSE}
bag_of_words2 <-  hm_data[which(hm_data$gender == "m"),] %>%
  unnest_tokens(word, text)
```
...
ui <- navbarPage("What makes people happy?",
                 ...
                 tabPanel("Mixture Analysis", 
                          titlePanel(h1("Most Frequent Occurrences",
                                        align = "center")),
                          
                          sidebarLayout(
                            sidebarPanel(
                              sliderInput(inputId = "Wordcloud",
                                          label = "Number of terms for word cloud:",
                                          min = 5,
                                          max = 100,
                                          value = 50),
                              ...
                              selectInput(inputId = "Parenthood",
                                          label = "Select the Parenthood:",
                                          choices = c("",
                                                      "No" = "n",
                                                      "Yes" = "y")),
                              br(),
...
server <- function(input, output, session) {
  ...
  output$MWC <- renderWordcloud2({
    ...
    if(input$Gender == "Male"){
      temp <- temp[which(gender = "m"),]
    }else if(input$Gender == "Female"){
      temp <- temp[which(gender = "f"),]
    }
    if(input$MS == "Single"){
      temp <- temp[which(marital = "single"),]
    }else if(input$MS == "Married"){
      temp <- temp[which(marital = "married"),]
    }
    if(input$Parenthood == "No"){
      temp <- temp[which(parenthood = "n"),]
    }else if(input$Parenthood == "Yes"){
      temp <- temp[which(parenthood = "y"),]
    }
    if(input$age == 18){
      temp <- temp[which(strtoi(temp$age) <= 18),]
    }else if(input$age == 25){
      temp <- temp[which(strtoi(temp$age) > 18 && strtoi(temp$age) <= 25),]
    }else if(input$age == 35){
      temp <- temp[which(strtoi(temp$age) > 25 && strtoi(temp$age) <= 35),]
    }else if(input$age == 50){
      temp <- temp[which(strtoi(temp$age) > 35 && strtoi(temp$age) <= 50),]
    }
    
    temp_WC <- temp %>%
      count(word, sort = TRUE)
    word_count %>%
      slice(1:input$Wordcloud) %>%
